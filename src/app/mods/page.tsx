  ...
  return (
    <div className='grid grid-cols-1 md:grid-cols-8 gap-6 pt-16'>
      ...
      <div className='w-full h-full md:col-span-6 rounded-2xl flex flex-col gap-4'>
        {mods.map(m => (<ProjectItemCard key={m.id} id={m.id} slug={m.slug} name={m.name} authorName={m.author.name ?? "Unknown"} description={m.description} numDownloads={m.downloads}/>))}
      </div>
    </div>
  );
};
